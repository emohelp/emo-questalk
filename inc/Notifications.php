...
class EMQA_Notifications {

	private $time_delay = 120;
	// private $time_delay = 0;
	public function __construct() {

		if(get_option('emqa_enable_email_delay')){
			add_action('emqa_new_question_notify', array( $this, 'new_question_notify' ), 10, 2);
			add_action('emqa_new_answer_notify', array( $this, 'new_answer_notify' ), 10, 2);
			add_action('emqa_new_comment_notify', array( $this, 'new_comment_notify' ), 10, 2);

			add_action( 'emqa_add_question', array( $this, 'emqa_queue_add_question' ), 10, 2 );
			add_action( 'emqa_add_answer', array( $this, 'emqa_queue_add_answer' ), 10, 2 );
			add_action( 'wp_insert_comment', array( $this, 'emqa_queue_insert_comment' ), 10, 2 );
		}else{
			add_action( 'emqa_add_question', array( $this, 'new_question_notify' ), 10, 2 );
			add_action( 'wp_insert_comment', array( $this, 'new_comment_notify' ), 10, 2 );
			add_action( 'emqa_add_answer', array( $this, 'new_answer_notify' ), 10, 2 );
		}

		// add_action( 'emqa_add_question', array( $this, 'new_activity' ) );
		// add_action( 'emqa_add_answer', array( $this, 'new_activity' ) );
		// add_action( 'emqa_add_comment', array( $this, 'new_activity' ) );
	}
	
	public function emqa_queue_add_question($question_id, $user_id){
		wp_schedule_single_event( time() + 120, 'emqa_new_question_notify', array($question_id, $user_id) );
	}
	public function emqa_queue_add_answer($answer_id, $question_id){
		wp_schedule_single_event( time() + 120, 'emqa_new_answer_notify', array($answer_id, $question_id) );
	}
	public function emqa_queue_insert_comment($comment_id, $comment){
		wp_schedule_single_event( time() + 120, 'emqa_new_comment_notify', array($comment_id, $comment) );
	}
	...
	public function new_question_notify( $question_id, $user_id ) {
		...
		$enabled = get_option( 'emqa_subscrible_enable_new_question_notification', 1 );
		if ( ! $enabled ) {
			return false;
		}
		...
		$message = emqa_get_mail_template( 'emqa_subscrible_new_question_email', 'new-question' );
		if ( ! $message ) {
			return false;
		}
		// Replacement
		...
		$message = str_replace( '{user_link}', emqa_get_author_link( $user_id ), $message );
		...
		// Site info
		$logo = get_option( 'emqa_subscrible_email_logo', '' );
	...
	public function new_answer_notify( $answer_id, $question_id ) {
		...
		$site_url = site_url();
		$enable_send_copy = get_option( 'emqa_subscrible_send_copy_to_admin' );
		$admin_email = $this->get_admin_email('answer');
		$site_logo = get_option( 'emqa_subscrible_email_logo', '' );
		$site_logo = $site_logo ? '<img src="' . $site_logo . '" alt="' . get_bloginfo( 'name' ) . '" style="max-width: 100%; height: auto;" />' : '';

		// for answer
		$answer_is_anonymous = emqa_is_anonymous( $answer_id );
		...
		// for question
		$question_is_anonymous = emqa_is_anonymous( $question_id );
		...
		if ( $answer_notify_enabled && !empty( $followers_email ) && is_array( $followers_email ) && 'private' !== get_post_status( $answer_id ) ) {
			...
			$message = emqa_get_mail_template( 'emqa_subscrible_new_answer_followers_email', 'new-answer-followers' );
			$message = apply_filters( 'emqa_get_new_answer_email_to_followers_message', $message, $answer_id, $question_id );
		...
		if ( $user_question_email && $answer_notify_for_question_enabled && absint( $user_answer_id ) !== absint( $user_question_id ) ) {
			...
			$message = emqa_get_mail_template( 'emqa_subscrible_new_answer_email', 'new-answer' );
			$message = apply_filters( 'emqa_get_new_answer_email_to_author_message', $message, $question_id, $answer_id );
			if ( !$message ) {
				return false;
			}
	...
	public function new_comment_notify( $comment_id, $comment ) {
		...
		if ( 1 == $comment->comment_approved && ( 'emqa-question' == $parent || 'emqa-answer' == $parent ) ) { 
			if ( $parent == 'emqa-question' ) {
				$enabled = get_option( 'emqa_subscrible_enable_new_comment_question_notification', 1 );
				// $admin_email = $this->get_admin_email( 'comment-question' );   //ignore in this time   
			} elseif ( $parent == 'emqa-answer' ) {
				$enabled = get_option( 'emqa_subscrible_enable_new_comment_answer_notification', 1 );
			...
			if ( emqa_is_anonymous( $comment->comment_post_ID ) ) {
				$post_parent_email = get_post_meta( $comment->comment_post_ID, '_emqa_anonymous_email', true );
				...
				if ( $post_parent->post_author != $comment->user_id ) {
					...
					if ( ! emqa_is_followed( $post_parent->ID, $comment->user_id ) ) {
						add_post_meta( $post_parent->ID, '_emqa_followers', $comment->user_id );
					}
				}
			...
			$message = str_replace( '{comment_author_link}', emqa_get_author_link( $comment->user_id ), $message );
			...
			$message = str_replace( '{site_url}', site_url(), $message );
			if ( $parent == 'emqa-question' ) {
				$enable_notify = get_option( 'emqa_subscrible_enable_new_comment_question_followers_notify', true );
			} else {
				$enable_notify = get_option( 'emqa_subscrible_enable_new_comment_answer_followers_notification', true );
			}
			
			if ( $enable_notify ) {
				//Follower email task
				$followers = get_post_meta( $post_parent->ID, '_emqa_followers' );
				...
				$message_to_follower = str_replace( '{comment_author_link}', emqa_get_author_link( $comment->user_id ), $message_to_follower );
	...
	public function get_admin_email( $type = 'question' ){
		switch ($type) {
			case 'answer':
				$admin_email = get_option( 'emqa_subscrible_new_answer_forward', '' );
				break;
			case 'comment-question':
				$admin_email = get_option( 'emqa_subscrible_new_comment_question_forward', '' );
				break;
			case 'comment-answer':
				$admin_email = get_option( 'emqa_subscrible_new_comment_answer_forward', '' );
				break;
			case 'question':
			default:
				$admin_email = get_option( 'emqa_subscrible_sendto_address', '' );
				break;
		}
		$emails = preg_split('/\r\n|\r|\n/', $admin_email );
	...
	function new_activity( $post_id = 0 ) {
		...
		if ( 'emqa_add_comment' == current_action() ) {
			$comment = get_comment( $post_id );
			$title = __( 'New Comment in: ', 'emqa-notification' );
		...
		if ( 'emqa-answer' == get_post_type( $post_id ) ) {
			...
			if ( !$title ) {
				$title = __( 'New Answer in: ', 'emqa-notification' );
			}

		} else {
			$question_id = $post_id;
			if ( !$title ) {
				$title = __( 'New Question: ', 'emqa-notification' );
			}
		}
		...
		$args = array( 'title' => $title, 'message' => wp_strip_all_tags( $content ) );
		$this->push( $args );
	}

	public function get_from_address() {
		$from_email = get_option( 'emqa_subscrible_from_address', get_bloginfo( 'admin_email' ) );
	...
	public function get_from_name() {
		$name = get_option( 'emqa_subscrible_from_name', get_bloginfo( 'name' ) );
	...
	public function get_content_type() {
		return apply_filters( 'emqa_notifications_get_content_type', 'text/html' );
	}
