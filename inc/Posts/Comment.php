...
class EMQA_Posts_Comment {
	public function __construct() {
		...
		add_filter( 'emqa_pre_comment_content', 'wp_kses_data', 10 );
	}
	/**
	...
	public function hook_redirect_comment_for_answer( $location, $comment ) {
		...
		if ( 'emqa-answer' == get_post_type( $comment->comment_post_ID ) ) {
			$question_id = emqa_get_post_parent_id( $comment->comment_post_ID );
			if ( $question_id ) {
	...
	/**
	 * Displays form token for unfiltered comments. Override wp_comment_form_unfiltered_html_nonce custom for emqa
	 *
	...
	public function wp_comment_form_unfiltered_html_nonce() {
		...
		if ( current_user_can( 'unfiltered_html' ) 
				&& 'emqa-answer' != get_post_type( $post_id )  ) {
	...
	public function comment_form_id_fields_filter( $result, $id, $replytoid ) {
		if ( 'emqa-answer' == get_post_type( $id ) ) {
	...
	public function sanitizie_comment( $content, $comment ) {
		...
		if ( $post_type == 'emqa-question' || $post_type == 'emqa-answer' ) {
	...
	public function get_comments() {
		if ( isset( $_GET['post'] ) ) {
			...
			wp_list_comments( array( 
				'style' => 'ol',
				'callback'  => 'emqa_question_comment_callback',
			), $comments ); 
		}
		exit( 0 );
	}
	...
	public function reopen_question_have_new_comment( $comment_ID ){
		...
		$question_id = false;
		if ( 'emqa-answer' == $comment_post_type ) {
			$question_id = emqa_get_post_parent_id( $comment->comment_post_ID );
		} elseif ( 'emqa-question' == $comment_post_type ) {
		...
		if ( $question_id ) {
			...
			if ( ! user_can( $comment->user_id, 'edit_posts' ) ) {
				if ( 'resolved' == $question_status ) {
					update_post_meta( $question_id, '_emqa_status', 're-open' );
				}
			}
		}
