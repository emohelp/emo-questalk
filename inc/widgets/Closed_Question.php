...
class EMQA_Widgets_Closed_Question extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array( 'classname' => 'emqa-widget emqa-closed-questions', 'description' => __( 'Show a list of questions marked as closed.', 'em-question-answer' ) );
		parent::__construct( 'emqa-closed-question', __( 'EMQA Closed Questions', 'em-question-answer' ), $widget_ops );
	}

	function widget( $args, $instance ) {
		extract( $args, EXTR_SKIP );
		$instance = wp_parse_args( $instance, array( 
			'title' => __( 'Closed Questions', 'em-question-answer' ),
			'number' => 5,
		) );
		...
		if ( $questions->have_posts() ) {
			...
			while ( $questions->have_posts() ) { $questions->the_post( );
				echo '
				<li><a href="'.get_permalink( ).'" class="question-title">'.get_the_title( ).'</a> '.__( 'asked by' , 'em-question-answer' ).' '. get_the_author_link( );
				'</li>';
			}   
			echo '</ul>';
			echo '</div>';
