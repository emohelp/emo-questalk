...
class EMQA_Widgets_Latest_Question extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array( 'classname' => 'emqa-widget emqa-latest-questions', 'description' => __( 'Show a list of latest questions.', 'em-question-answer' ) );
		parent::__construct( 'emqa-latest-question', __( 'EMQA Latest Questions', 'em-question-answer' ), $widget_ops );
	}

	function widget( $args, $instance ) {
		extract( $args, EXTR_SKIP );
		$instance = wp_parse_args( $instance, array( 
			'title' => __( 'Latest Questions' , 'em-question-answer' ),
			'number' => 5,
		) );
		...
		if ( $questions->have_posts() ) {
			...
			while ( $questions->have_posts() ) { $questions->the_post( );
				...
				the_title();
				echo '</a>';
				echo ' '.__( 'asked by', 'em-question-answer' ) . ' ' . get_the_author_link();
				if ( isset( $instance['question_date'] ) && $instance['question_date'] ) {
					// translators: %s is replaced with the login URL
					echo ', ' . sprintf( esc_html__( '%s ago', 'em-question-answer' ), human_time_diff( get_post_time('U', true, get_the_ID() ) ) ) . '.';
				}
				echo '</li>';
			}   
			echo '</ul>';
	...
	function form( $instance ) {
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'question_date' ) ?>"><?php _e( 'Show question date', 'em-question-answer' ) ?></label>
		</p>
		<?php
	}
}
