...
<div class="emqa-comment">
    ...
    _x( 'replied %s ago', '%s = human-readable time difference', 'emqa' ),
    human_time_diff( get_comment_time( 'U', true ) )
		); ?>

		<div class="emqa-comment-actions">
			<?php if ( emqa_current_user_can( 'edit_comment' ) ) : ?>
				<a href="<?php echo esc_url( add_query_arg( array( 'comment_edit' => $comment->comment_ID ) ) ) ?>"><?php _e( 'Edit', 'emqa' ) ?></a>
			<?php endif; ?>
			<?php if ( emqa_current_user_can( 'delete_comment' ) ) : ?>
				<a class="emqa-delete-comment" href="<?php echo wp_nonce_url( add_query_arg( array( 'action' => 'emqa-action-delete-comment', 'comment_id' => $comment->comment_ID ), admin_url( 'admin-ajax.php' ) ), '_emqa_delete_comment' ) ?>"><?php _e( 'Delete', 'emqa' ) ?></a>
			<?php endif; ?>
		</div>
	</div>
	<?php comment_text(); ?>
