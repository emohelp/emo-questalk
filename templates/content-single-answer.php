...
<div class="<?php echo emqa_post_class() ?>">
	...
	<?php if ( emqa_current_user_can( 'edit_question', emqa_get_question_from_answer_id() ) ) : ?>
		...
		<a class="emqa-pick-best-answer" href="<?php echo esc_url( wp_nonce_url( add_query_arg( array( 'answer' => get_the_ID(), 'action' => $action ), admin_url( 'admin-ajax.php' ) ), '_emqa_vote_best_answer' ) ) ?>"><?php _e( 'Best Answer', 'em-question-answer' ) ?></a>
	<?php elseif ( emqa_is_the_best_answer() ) : ?>
		<span class="emqa-pick-best-answer"><?php _e( 'Best Answer', 'em-question-answer' ) ?></span>
	<?php endif; ?>
	<div class="emqa-answer-meta">
		...
		<?php printf(
			...
			( '<span><a href="%1$s">%2$s%3$s</a> %4$s answered %5$s '.__('ago', 'em-question-answer').'</span>' ),
		...
		<?php if ( 'private' == get_post_status() ) : ?>
			<span><?php _e( '&nbsp;&bull;&nbsp;', 'em-question-answer' ); ?></span>
			<span><?php _e( 'Private', 'em-question-answer' ) ?></span>
		<?php endif; ?>
