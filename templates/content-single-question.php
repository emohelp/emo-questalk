...
<div class="emqa-question-item">
    ...
    ( '<span><a href="%1$s">%2$s%3$s</a> %4$s asked %5$s '.__('ago', 'emqa').'</span>'),
		...
		<div class="emqa-question-meta">
			...
			<?php if ( emqa_current_user_can( 'edit_question', get_the_ID() ) ) : ?>
				...
				<span class="emqa-question-status">
					<?php _e( 'This question is:', 'emqa' ) ?>
					<select id="emqa-question-status" data-nonce="<?php echo wp_create_nonce( '_emqa_update_privacy_nonce' ) ?>" data-post="<?php the_ID(); ?>">
						<optgroup label="<?php _e( 'Status', 'emqa' ); ?>">
							<option <?php selected( emqa_question_status(), 'open' ) ?> value="open"><?php _e( 'Open', 'emqa' ) ?></option>
							<option <?php selected( emqa_question_status(), 'closed' ) ?> value="closed"><?php _e( 'Closed', 'emqa' ) ?></option>
							<option <?php selected( emqa_question_status(), 'resolved' ) ?> value="resolved"><?php _e( 'Resolved', 'emqa' ) ?></option>
						</optgroup>
					</select>
					</span>
				<?php endif; ?>
